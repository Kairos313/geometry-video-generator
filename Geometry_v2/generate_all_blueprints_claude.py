...
def generate_all_blueprints_claude():
    ...
    for idx, step in enumerate(steps):
        scene_id = step.get('scene_id', f'scene_{idx}')
        out_path = output_dir / f"scene_{idx}_{scene_id}.json"
        if out_path.exists():
            logger.info(f"[{idx+1}/{total}] Skipping scene {idx} ({scene_id}) - already exists.")
            continue
        ...
        print(f"\n=== [{idx+1}/{total}] CLAUDE SONNET API: Scene {idx} ({scene_id}) ===")
        ...
        try:
            ...
            if blueprint:
                ...
                raw_path = output_dir / f"scene_{idx}_{scene_id}_raw.txt"
